    ...
    <>
      <nav className="w-full bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {/* Left - Brand */}
          <Link to="/" className="text-2xl font-black tracking-tighter uppercase text-black z-50">
            VANTAWEAR
          </Link>
          ...
          <div className="hidden md:block flex-1 max-w-md mx-8">
            <form onSubmit={handleSearch} className="relative flex items-center w-full group">
              ...
              />
              <button type="submit" className="absolute right-2 p-1 text-gray-400 group-focus-within:text-black transition-colors hover:scale-110">
                <SearchIcon size={18} />
              </button>
            </form>
          </div>
      ...
      >
        {/* Mobile Search Form */}
        <form onSubmit={handleSearch} className="relative flex items-center w-full mb-12">
          <input
            type="text"
            value={searchInput}
            onChange={(e) => setSearchInput(e.target.value)}
          ...
          />
          <button type="submit" className="absolute right-0 p-2 text-black hover:scale-110 transition-transform">
            <SearchIcon size={24} />
          </button>
        </form>
        ...
        <div className="flex flex-col gap-8 text-3xl font-black uppercase tracking-tighter">
          <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-gray-500 transition-colors">
            Home
          </Link>
          <Link to="/shop" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-gray-500 transition-colors">
            Shop All
          </Link>
          <div className="border-t border-gray-200 my-4"></div>
          {/* Example Category Links */}
          <Link to="/category/hoodies" onClick={() => setIsMobileMenuOpen(false)} className="text-xl text-gray-500 hover:text-black transition-colors">
            Hoodies
          </Link>
          <Link to="/category/tees" onClick={() => setIsMobileMenuOpen(false)} className="text-xl text-gray-500 hover:text-black transition-colors">
            Tees
          </Link>
          <Link to="/category/bottoms" onClick={() => setIsMobileMenuOpen(false)} className="text-xl text-gray-500 hover:text-black transition-colors">
            Bottoms
          </Link>
        </div>
